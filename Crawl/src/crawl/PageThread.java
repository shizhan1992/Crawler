...
public class PageThread implements Runnable {
	...
	public PageThread(TagNode tag, String frameUrl, String code,
			CrawlTime crawltime) {
	...
	public void run() {
		...
		tempTopic.topicClick = Long.parseLong(topicClick.toPlainTextString());
		...
		// 3.发帖人
		Node topicPublisher = topicClick.getNextSibling().getNextSibling()
				.getNextSibling();
		...
		String htmlcode = commentpage.downloadpage(frameUrl.substring(0,
				frameUrl.length() - 5)
				+ ",d.html");
		if (htmlcode != null) {
		...
		if (tempTopic.updateDate.after(crawltime.START_TIME)
				&& tempTopic.updateDate.before(crawltime.END_TIME)) {
			// 判断是否在当前爬取时间范围内
			if (tempTopic.topicPublishDate.after(crawltime.INITIAL_TIME)) {
				...
				if (tempTopic.topicComment > 3)
					AllStock.tt.execute(new CommentThread(tempTopic, code,
							crawltime));
