...
public class CommentThread implements Runnable {
	...
	public void run() {
		int pagenum = 1;
		do {
			String commentPage = topicUri.substring(0, topicUri.length() - 5)
					+ ",d_" + pagenum + ".html";
			...
			if (htmlcode != null) {
				...
				if (list.size() > 0) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						// 提取主内容node
						Node node = list.elementAt(i).getLastChild()
						...
						if (node.getText().indexOf("zwlitxt") > -1) {
							...
							// 评论内容节点
							Node ComStrTag = node.getLastChild()
							...
							// 评论日期节点
							Node comdate = ComStrTag.getNextSibling()
									.getNextSibling().getFirstChild()
									.getNextSibling();
							if (comdate.getText().indexOf("zwlitxb") > -1) {
								String commentDateStr = comdate
										.toPlainTextString();
							...
							Comment tempComment = new Comment(commentDate,
									topicUri, commentStr, this.title, p);
